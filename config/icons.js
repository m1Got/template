...
const iconsSrcFolder = paths.resources + "/icons";
...
async function svgToFont() {
  try {
    ...
    removingFolders.forEach((path) => fs.existsSync(path) && fs.rmdirSync(path));
    removingFiles.forEach((path) => fs.existsSync(path) && fs.unlinkSync(path));

    fs.renameSync(`${iconsDistFolder}/icons.symbol.svg`, `${iconsDistFolder}/sprite.svg`);
